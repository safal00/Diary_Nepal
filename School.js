...
const SHEET_GID = "1522272416"; // replace with Airlines tab GID
...
console.log("🏫 Schools & Colleges Directory loaded");
...
fetch(url)
  ...
  .then(text => {
    ...
    .filter(r => r["Province"] || r["District"] || r["School Name"] || r["Phone"]);

    console.log(`✅ Loaded ${allRows.length} schools/colleges`);
    initFilters();
    applyFilters();
  })
  .catch(err => console.error("❌ Error loading Schools sheet:", err));
