...
function renderResults(data){
  ...
  if(!data.length){ resultsTableBody.innerHTML=`<tr><td colspan="6">No results found.</td></tr>`; return; }
  ...
  data.forEach(r=>{
    ...
    if(r["Email"]){ const a=document.createElement("a"); a.href=`mailto:${r["Email"]}`; a.textContent=r["Email"]; emailCell.appendChild(a); } else emailCell.textContent="-";
    row.appendChild(emailCell);

    const websiteCell=document.createElement("td");
    if(r["Website"]){ const a=document.createElement("a"); a.href=r["Website"].startsWith("http")?r["Website"]:`https://${r["Website"]}`; a.target="_blank"; a.textContent=r["Website"]; websiteCell.appendChild(a); } else websiteCell.textContent="-";
    row.appendChild(websiteCell);

    const locationCell=document.createElement("td");
    locationCell.textContent=`${r["Local Level"]}, ${r["District"]}, ${r["Province"]}`; row.appendChild(locationCell);
